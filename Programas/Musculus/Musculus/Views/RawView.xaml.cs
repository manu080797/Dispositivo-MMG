...
namespace Musculus.Views
{
    ...
    public partial class RawView : ContentPage
    {
        ...
        {
            InitializeComponent();
            BindingContext = new RawViewModel();
        ...
        {
            base.OnAppearing();
            ((RawViewModel)BindingContext).Resume();
        }

        protected override void OnDisappearing()
        {
            base.OnDisappearing();
            ((RawViewModel)BindingContext).Suspend();
        }
        ...
        public void RenderPlotMMG(object sender, SKPaintSurfaceEventArgs args)
        {
            ...
            float scale = (height / 2.0f) / ((RawViewModel)BindingContext).MaxVoltage;
            float pixelOffsetZ1 = height / 2.0f;
            float x, y, y_mean;

            var dataBufferZ1 = ((RawViewModel)BindingContext).DataBufferZ1;
        ...
        public void RenderPlotForce(object sender, SKPaintSurfaceEventArgs args)
        {
            ...
            float maxForce = ((RawViewModel)BindingContext).MaxForce;
            float targetForce = (float)((RawViewModel)BindingContext).TargetForce;
            float targetForceBounds = (float)((RawViewModel)BindingContext).TargetForceBounds;
            ...
            var forceData = ((RawViewModel)BindingContext).DataBufferZ2.Data;
